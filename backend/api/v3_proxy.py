"""Proxy read-only v3 certificate requests to the EJBCA gateway."""
...
@v3_proxy_bp.route('/api/v3/certificates', defaults={'suffix': ''}, methods=['GET'])
@v3_proxy_bp.route('/api/v3/certificates/<path:suffix>', methods=['GET'])
def certificates_proxy(suffix):
    gateway_url = os.getenv('EJBCA_GATEWAY_URL', 'http://ejbca-gateway:8081').rstrip('/')
    path = '/api/v3/certificates' + (f'/{suffix}' if suffix else '')
